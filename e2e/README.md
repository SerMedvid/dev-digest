...
| `11-pr-brief` | PR #482 → Overview tab → the seeded PR brief: banner `why` + risk badge, not stale, a RISK AREAS row expanding to its explanation and its grounded ref, and the Review focus list — `:line` present only where a seeded finding covers it. Ends by clicking a focus row and landing on that file in the Files changed tab. Never regenerates, so no LLM call |
