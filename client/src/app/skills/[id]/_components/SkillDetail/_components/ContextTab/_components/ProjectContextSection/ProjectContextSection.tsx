...
export function ProjectContextSection({ skill }: { skill: Skill }) {
  ...
  // Attached, in THIS repository: the only rows with a stored order to drag.
  const draggable = visible.filter((row) => row.attached && row.repoId === null);
  const rest = visible.filter((row) => !(row.attached && row.repoId === null));
  ...
  return (
    ...
    >
      ...
      {rows.length === 0 ? (
        ...
        <>
          {/* Only this repository's attached rows have a stored position to
              move, and order is a real edit: it is the order a run assembles
              the documents in. `attachedPaths` is the replace payload, so the
              drag is indexed into that rather than into the filtered view. */}
          <AttachmentList
            ids={draggable.map((row) => row.path)}
            onReorder={(activeId, overId) => {
              const from = attachedPaths.indexOf(activeId);
              const to = attachedPaths.indexOf(overId);
              if (from < 0 || to < 0) return;
              commit(moveAttached(attachedPaths, from, to));
            }}
          ...
          {/* Keyed by repository *and* path: the same document may be attached
              in several repositories, and each of those is its own inert row
              (AC-50). */}
          {rest.map((row) => (
            <DocRow
