...
| `RiskAreas` | `IntentCard/_components/` | `brief.risks` rows with expandable explanations. Presentational — calls no hook |
...
| Generation conflicted (`409`) | `brief.conflict` — a distinct string, because "already running" is not a failure to retry |
...
- Severity and risk level are always spelled out in text. Colour is never the
  only carrier — the design system's rule, and the reason the intent card writes
  its confidence out too.
...
- [ ] A 409 renders the conflict message, not the generic failure message.
- [ ] `RiskAreas` renders one row per risk with its refs; the chevron expands the
      explanation; an empty array renders nothing.
