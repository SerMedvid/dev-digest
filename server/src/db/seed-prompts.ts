 ...
 *
 * `test/seed-prompts.test.ts` enforces the mirror — edit one side only and it
 * fails, naming the file and the constant.
 */
...
/**
 * The Test Quality Reviewer is deliberately a SHELL: role, severity, verdict,
 * findings discipline, and nothing about what to look for. Its subject matter
 * arrives at run time from the skills linked in `./seed-skills.ts`, rendered
 * under `## Skills / rules`. Adding a check here instead of there defeats the
 * point — the design is that a rule is editable in the Skills library with no
 * deploy. See `docs/superpowers/specs/2026-08-02-test-quality-reviewer-design.md`.
 */
